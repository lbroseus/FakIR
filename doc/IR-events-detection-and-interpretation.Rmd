---
title: "IR events detection and interpretation using IRFindeR2"
...
The R package *IRFindeR2* can be installed from my gitlab account by copy-pasting the following code line:
...
Once you have installed *IRFindeR2*, these files can be found in folder:
...
*MCF10A* data available in *IRFindeR2* package consists in:
...
IRFindeR2 includes a wrapper to perform end-to-end differential analysis of introns through DEXSeq.  
...
To carry out differential analysis of IR-levels using IRFindeR2, you will need:
...
This function will first identify intron intervals from the input reference transcriptome (gtf) by calling the function ['IRFindeR2::createIntronGR()'] and will then select all long reads whose spliced alignment overlaps an intron.   
