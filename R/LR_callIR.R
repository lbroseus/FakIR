...
#' of an input genomic interval that is contained in a read.
#'
...
#'
#' @param bamFile Path to the long read alignments (bam file) from which to call IR events
...
callIRevents <- function(bamFile, intronGR, flankWidth = 20, keepSecondaryAlignments = FALSE){
  ...
  seqlevelsStyle(bam) <- seqlevelsStyle(intronGR) <- "UCSC"
  ...
  start_time <- Sys.time()
  result <- tapply(1:length(bam), 1:length(bam), function(i) computeIntronOverlap(bam[i], intronGR[i]))
  end_time <- Sys.time()
  ...
  flank_hits <- findOverlaps(query = bam, subject = intron_flank, minoverlap = flankWidth)
